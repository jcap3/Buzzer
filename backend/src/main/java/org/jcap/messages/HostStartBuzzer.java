...
public class HostStartBuzzer implements Message {
    @Override
    public String processMessage(Client client, SimpleMessage simpleMessage) {
        ...
        for (Integer integer : gameData.getAllClientsInThisGameData()) {
            FrontEndBuzzerQueueEndpoint.sendToSpecificSession(integer, new SimpleMessage(MessageTypes.HOST_START_BUZZER, null));
        }
        return "success";
    }
